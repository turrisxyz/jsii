...
public final class JsiiEngine implements JsiiCallbackHandler {
     ...
     * Object cache.
     */
    private final Map<JsiiObjectRef, Object> objects = new HashMap<>();

    /**
     * The jsii-server child process.
     */
    private final JsiiClient client = new JsiiClient();
    ...
    public void registerObject(final JsiiObjectRef objectRef, final Object value) {
        if (objectRef != null) {
            this.objects.put(objectRef, value);
            if (value instanceof JsiiObject) {
                final JsiiObject jsiiValue = (JsiiObject) value;
                if (jsiiValue.jsii$objRef == null) {
                    jsiiValue.jsii$objRef = objectRef;
                }
            }
        }
    }
    ...
    public Object getObject(final JsiiObjectRef objRef) {
        Object obj = this.objects.get(objRef);
        if (obj == null) {
            throw new JsiiException("Cannot find jsii object: " + objRef);
        }
        return obj;
    }

    public <T> T getObject(final JsiiObjectRef objRef, final NativeType<T> nativeType) {
        final Object obj = this.objects.get(objRef);
        if (obj != null) {
            return nativeType.transform(obj);
        }
        final T proxy = nativeType.newProxy(objRef);
        this.registerObject(objRef, proxy);
        return proxy;
    }
