...
fn create_snapshot(mut js_runtime: JsRuntime, snapshot_path: &Path, files: Vec<PathBuf>) {
  ...
  let compressed_snapshot_with_size = {
    ...
    vec.extend_from_slice(&zstd::block::compress(snapshot_slice, 22).expect("snapshot compression failed"));
...
fn create_compiler_snapshot(snapshot_path: &Path, files: Vec<PathBuf>, cwd: &Path) {
  ...
  op_crate_libs.insert("deno.broadcast_channel", deno_broadcast_channel::get_declaration());
  ...
  js_runtime.register_op("op_cwd", op_sync(move |_state, _args: Value, _: ()| Ok(json!("cache:///"))));
  ...
  js_runtime.register_op("op_exists", op_sync(move |_state, _args: LoadArgs, _: ()| Ok(json!(false))));
...
fn ts_version() -> String {
  std::fs::read_to_string("src/tsc/00_typescript.js")
    .unwrap()
    .lines()
...
fn git_commit_hash() -> String {
  if let Ok(output) = std::process::Command::new("git").arg("rev-list").arg("-1").arg("HEAD").output() {
...
fn main() {
  ...
  println!("cargo:rustc-env=DENO_URL_LIB_PATH={}", deno_url::get_declaration().display());
  println!("cargo:rustc-env=DENO_WEB_LIB_PATH={}", deno_web::get_declaration().display());
  println!("cargo:rustc-env=DENO_FETCH_LIB_PATH={}", deno_fetch::get_declaration().display());
  println!("cargo:rustc-env=DENO_WEBGPU_LIB_PATH={}", deno_webgpu_get_declaration().display());
  println!(
  ...
  println!("cargo:rustc-env=DENO_CRYPTO_LIB_PATH={}", deno_crypto::get_declaration().display());
  println!(
    "cargo:rustc-env=DENO_BROADCAST_CHANNEL_LIB_PATH={}",
    deno_broadcast_channel::get_declaration().display()
  );
  println!("cargo:rustc-env=DENO_NET_LIB_PATH={}", deno_net::get_declaration().display());
  ...
  let js_files = get_js_files("src/tsc");
...
fn deno_webgpu_get_declaration() -> PathBuf {
  ...
  manifest_dir.join("src/dts").join("lib.deno_webgpu.d.ts")
}
