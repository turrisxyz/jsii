...
pub use deno_runtime::permissions::Permissions;
...
fn create_web_worker_callback(ps: ProcState) -> Arc<CreateWebWorkerCb> {
  Arc::new(move |args| {
    ...
    let module_loader = CliModuleLoader::new_for_worker(ps.clone(), args.parent_permissions.clone());
    ...
    let options = WebWorkerOptions {
      ...
      extensions,
      unsafely_ignore_certificate_errors: ps.flags.unsafely_ignore_certificate_errors.clone(),
...
fn print_cache_info(state: &ProcState, json: bool, location: Option<&deno_core::url::Url>) -> Result<(), AnyError> {
  let deno_dir = &state.dir.root;
  let modules_cache = &state.file_fetcher.get_http_cache_location();
  let typescript_cache = &state.dir.gen_cache.location;
  let registry_cache = &state.dir.root.join(lsp::language_server::REGISTRIES_PATH);
  ...
  if json {
    ...
    println!("{} {:?}", colors::bold("Emitted modules cache:"), typescript_cache);
    println!("{} {:?}", colors::bold("Language server registries cache:"), registry_cache,);
    println!("{} {:?}", colors::bold("Origin storage:"), origin_dir);
    if location.is_some() {
      println!("{} {:?}", colors::bold("Local Storage:"), origin_dir.join("local_storage"),);
    }
    Ok(())
  }
}
...
async fn compile_command(flags: Flags, compile_flags: CompileFlags) -> Result<i32, AnyError> {
  ...
  let run_flags = tools::standalone::compile_to_runtime_flags(&flags, compile_flags.args.clone())?;

  let module_specifier = resolve_url_or_path(&compile_flags.source_file)?;
  let ps = ProcState::build(Arc::new(flags)).await?;
  let deno_dir = &ps.dir;

  let output = compile_flags
    .output
    .as_ref()
    .and_then(|output| {
    ...
    .or_else(|| infer_name_from_url(&module_specifier).map(PathBuf::from))
    .ok_or_else(|| generic_error("An executable name was not provided. One could not be inferred from the URL. Aborting."))?;

  let graph = Arc::try_unwrap(create_graph_and_maybe_check(module_specifier.clone(), &ps, debug).await?)
    .map_err(|_| generic_error("There should only be one reference to ModuleGraph"))?;

  graph.valid().unwrap();

  let eszip = eszip::EszipV2::from_graph(graph, Default::default())?;

  info!("{} {}", colors::green("Compile"), module_specifier.to_string());

  // Select base binary based on target
  let original_binary = tools::standalone::get_base_binary(deno_dir, compile_flags.target.clone()).await?;

  let final_bin = tools::standalone::create_standalone_binary(original_binary, eszip, module_specifier.clone(), run_flags, ps).await?;

  info!("{} {}", colors::green("Emit"), output.display());

  tools::standalone::write_standalone_binary(output.clone(), compile_flags.target, final_bin).await?;
...
async fn info_command(flags: Flags, info_flags: InfoFlags) -> Result<i32, AnyError> {
  ...
  if let Some(specifier) = info_flags.file {
    ...
    let maybe_resolver = if maybe_jsx_resolver.is_some() {
      ...
      maybe_import_map_resolver.as_ref().map(|im| im.as_resolver())
    };
...
async fn install_command(flags: Flags, install_flags: InstallFlags) -> Result<i32, AnyError> {
  ...
  let mut worker = create_main_worker(&ps, main_module.clone(), permissions, vec![]);
...
async fn uninstall_command(uninstall_flags: UninstallFlags) -> Result<i32, AnyError> {
...
async fn lint_command(flags: Flags, lint_flags: LintFlags) -> Result<i32, AnyError> {
  if lint_flags.rules {
...
async fn eval_command(flags: Flags, eval_flags: EvalFlags) -> Result<i32, AnyError> {
  ...
  let main_module = resolve_url_or_path(&format!("./$deno$eval.{}", eval_flags.ext)).unwrap();
...
async fn create_graph_and_maybe_check(
  ...
  let check_js = ps.maybe_config_file.as_ref().map(|cf| cf.get_check_js()).unwrap_or(false);
  ...
  if ps.flags.check != CheckFlag::None {
    ...
    let (ts_config, maybe_ignored_options) = emit::get_ts_config(
      emit::ConfigType::Check { tsc_emit: false, lib },
    ...
    let maybe_config_specifier = ps.maybe_config_file.as_ref().map(|cf| cf.specifier.clone());
...
fn bundle_module_graph(graph: &deno_graph::ModuleGraph, ps: &ProcState, flags: &Flags) -> Result<(String, Option<String>), AnyError> {
  info!("{} {}", colors::green("Bundle"), graph.roots[0].0);

  let (ts_config, maybe_ignored_options) = emit::get_ts_config(emit::ConfigType::Bundle, ps.maybe_config_file.as_ref(), None)?;
...
fn human_size(size: f64) -> String {
  ...
  let pretty_bytes = format!("{:.2}", size / delimiter.powi(exponent)).parse::<f64>().unwrap() * 1_f64;
...
async fn bundle_command(flags: Flags, bundle_flags: BundleFlags) -> Result<i32, AnyError> {
  ...
  let resolver = |_| {
    ...
    async move {
      ...
      if let Ok(Some(import_map_path)) =
        config_file::resolve_import_map_specifier(ps.flags.import_map_path.as_deref(), ps.maybe_config_file.as_ref())
  ...
  let operation = |(ps, graph): (ProcState, Arc<deno_graph::ModuleGraph>)| {
    ...
    async move {
      let (bundle_emit, maybe_bundle_map) = bundle_module_graph(graph.as_ref(), &ps, &ps.flags)?;
      debug!(">>>>> bundle END");
...
async fn doc_command(flags: Flags, doc_flags: DocFlags) -> Result<i32, AnyError> {
  tools::doc::print_docs(flags, doc_flags).await?;
  Ok(0)
}

async fn format_command(flags: Flags, fmt_flags: FmtFlags) -> Result<i32, AnyError> {
  let ps = ProcState::build(Arc::new(flags)).await?;
  ...
  if fmt_flags.files.len() == 1 && fmt_flags.files[0].to_string_lossy() == "-" {
    tools::fmt::format_stdin(fmt_flags, maybe_fmt_config.map(|c| c.options).unwrap_or_default())?;
    return Ok(0);
  }
...
async fn repl_command(flags: Flags, repl_flags: ReplFlags) -> Result<i32, AnyError> {
...
async fn run_from_stdin(flags: Flags) -> Result<i32, AnyError> {
  ...
  let mut worker = create_main_worker(&ps.clone(), main_module.clone(), permissions, vec![]);
...
async fn run_with_watch(flags: Flags, script: String) -> Result<i32, AnyError> {
  ...
  impl Drop for FileWatcherModuleExecutor {
    fn drop(&mut self) {
      if self.pending_unload {
        self.worker.dispatch_unload_event(&located_script_name!()).unwrap();
      }
    }
  }
  ...
  let operation = |(ps, main_module): (ProcState, ModuleSpecifier)| {
    ...
    async move {
      ...
      // operation is called.
      let mut executor = FileWatcherModuleExecutor::new(create_main_worker(&ps, main_module.clone(), permissions, vec![]), flags.compat);
...
async fn run_command(flags: Flags, run_flags: RunFlags) -> Result<i32, AnyError> {
  ...
  let mut maybe_coverage_collector = if let Some(ref coverage_dir) = ps.coverage_dir {
    ...
    worker.with_event_loop(coverage_collector.start_collecting().boxed_local()).await?;
  ...
  worker.dispatch_load_event(&located_script_name!())?;
  worker.run_event_loop(maybe_coverage_collector.is_none()).await?;
  worker.dispatch_unload_event(&located_script_name!())?;
...
async fn test_command(flags: Flags, test_flags: TestFlags) -> Result<i32, AnyError> {
  if let Some(ref coverage_dir) = flags.coverage_dir {
    ...
    env::set_var("DENO_UNSTABLE_COVERAGE_DIR", PathBuf::from(coverage_dir).canonicalize()?);
  }
...
async fn completions_command(_flags: Flags, completions_flags: CompletionsFlags) -> Result<i32, AnyError> {
...
async fn upgrade_command(_flags: Flags, upgrade_flags: UpgradeFlags) -> Result<i32, AnyError> {
...
fn get_subcommand(flags: Flags) -> Pin<Box<dyn Future<Output = Result<i32, AnyError>>>> {
  match flags.subcommand.clone() {
    ...
    DenoSubcommand::Doc(doc_flags) => doc_command(flags, doc_flags).boxed_local(),
    ...
    DenoSubcommand::Compile(compile_flags) => compile_command(flags, compile_flags).boxed_local(),
    DenoSubcommand::Coverage(coverage_flags) => coverage_command(flags, coverage_flags).boxed_local(),
    DenoSubcommand::Fmt(fmt_flags) => format_command(flags, fmt_flags).boxed_local(),
    DenoSubcommand::Info(info_flags) => info_command(flags, info_flags).boxed_local(),
    DenoSubcommand::Install(install_flags) => install_command(flags, install_flags).boxed_local(),
    DenoSubcommand::Uninstall(uninstall_flags) => uninstall_command(uninstall_flags).boxed_local(),
    DenoSubcommand::Lsp => lsp_command().boxed_local(),
    DenoSubcommand::Lint(lint_flags) => lint_command(flags, lint_flags).boxed_local(),
    DenoSubcommand::Repl(repl_flags) => repl_command(flags, repl_flags).boxed_local(),
    ...
    DenoSubcommand::Completions(completions_flags) => completions_command(flags, completions_flags).boxed_local(),
    DenoSubcommand::Types => types_command(flags).boxed_local(),
    DenoSubcommand::Upgrade(upgrade_flags) => upgrade_command(flags, upgrade_flags).boxed_local(),
...
pub fn main() {
  ...
  let exit_code = async move {
    let standalone_res = match standalone::extract_standalone(args.clone()).await {
    ...
    let flags = match flags::flags_from_vec(args) {
      Ok(flags) => flags,
      Err(err @ clap::Error { .. }) if err.kind == clap::ErrorKind::DisplayHelp || err.kind == clap::ErrorKind::DisplayVersion => {
        err.print().unwrap();
        std::process::exit(0);
      }
