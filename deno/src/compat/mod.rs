...
static GLOBAL_URL_STR: Lazy<String> = Lazy::new(|| format!("{}node/global.ts", NODE_COMPAT_URL.as_str()));

pub static GLOBAL_URL: Lazy<Url> = Lazy::new(|| Url::parse(&GLOBAL_URL_STR).unwrap());

static MODULE_URL_STR: Lazy<String> = Lazy::new(|| format!("{}node/module.ts", NODE_COMPAT_URL.as_str()));

pub static MODULE_URL: Lazy<Url> = Lazy::new(|| Url::parse(&MODULE_URL_STR).unwrap());

static COMPAT_IMPORT_URL: Lazy<Url> = Lazy::new(|| Url::parse("flags:compat").unwrap());
...
fn try_resolve_builtin_module(specifier: &str) -> Option<Url> {
  if SUPPORTED_MODULES.contains(&specifier) {
    let module_url = format!("{}node/{}.ts", NODE_COMPAT_URL.as_str(), specifier);
...
pub fn load_cjs_module(js_runtime: &mut JsRuntime, module: &str, main: bool) -> Result<(), AnyError> {
...
pub(crate) fn add_global_require(js_runtime: &mut JsRuntime, main_module: &str) -> Result<(), AnyError> {
  let source_code = &format!(
...
pub fn setup_builtin_modules(js_runtime: &mut JsRuntime) -> Result<(), AnyError> {
...
pub async fn translate_cjs_to_esm(
  ...
  for (idx, reexport) in analysis.reexports.iter().enumerate() {
    ...
    // Now perform analysis again
    {
      ...
      source.push(format!("const reexport{} = require(\"{}\");", idx, reexport));
